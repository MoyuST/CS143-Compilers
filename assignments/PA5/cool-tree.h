...
public:
   ...
   virtual Symbol get_name_on_condition(bool option) = 0;
   virtual Symbol get_type_on_condition(bool option) = 0;
   virtual bool is_attribute() = 0;
   virtual void set_idx(size_t i) = 0;

   size_t idx_inside_class;
...
public:
   ...
   Class_ copy_Class_();
   void dump(ostream& stream, int n);
   std::vector<Symbol> get_method_names(){
      std::vector<Symbol> rt_symbols;
      for(int i = features->first(); features->more(i); i = features->next(i)){
         Symbol rt = features->nth(i)->get_name_on_condition(true);
         if(rt!=NULL){
            rt_symbols.push_back(rt);
         }
      }
      return rt_symbols;
   }

   void setting_attr_offset(){
      size_t cur_attr_idx=1;
      for(int i = features->first(); features->more(i); i = features->next(i)){
         if(features->nth(i)->is_attribute()){
            features->nth(i)->set_idx(2+cur_attr_idx++);
         }
      }
   }

   std::vector<Symbol> get_attribute_names(){
      std::vector<Symbol> rt_symbols;
      for(int i = features->first(); features->more(i); i = features->next(i)){
         Symbol rt = features->nth(i)->get_name_on_condition(false);
         if(rt!=NULL){
            rt_symbols.push_back(rt);
         }
      }
   ...
   std::vector<Symbol> get_method_types(){
      std::vector<Symbol> rt_symbols;
      for(int i = features->first(); features->more(i); i = features->next(i)){
         Symbol rt = features->nth(i)->get_type_on_condition(true);
         if(rt!=NULL){
            rt_symbols.push_back(rt);
         }
      }
   ...
   std::vector<Symbol> get_attribute_types(){
      std::vector<Symbol> rt_symbols;
      for(int i = features->first(); features->more(i); i = features->next(i)){
         Symbol rt = features->nth(i)->get_type_on_condition(false);
         if(rt!=NULL){
            rt_symbols.push_back(rt);
         }
      }
...
public:
   ...
   void dump(ostream& stream, int n);
   Symbol get_name_on_condition(bool option){
      if(option){
         return name;
      }
      else{
   ...
   Symbol get_type_on_condition(bool option){
      if(option){
         return return_type;
      }
      else{
   ...
   void set_idx(size_t i) {idx_inside_class=i;}
...
public:
   ...
   size_t nt() {return pred->nt()+then_exp->nt()+else_exp->nt();}
#ifdef Expression_SHARED_EXTRAS
   Expression_SHARED_EXTRAS
#endif
#ifdef cond_EXTRAS
...
public:
   ...
   size_t nt() {return pred->nt()+body->nt();}
#ifdef Expression_SHARED_EXTRAS
   Expression_SHARED_EXTRAS
#endif
#ifdef loop_EXTRAS
